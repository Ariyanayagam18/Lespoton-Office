...
case 'bird_filter':
    ...
    if ($checkUser == 1) {
        if ($raceId != "" && $fancierId != "") {
            $get_race_details = mysqli_query($dbconnection, "select * from ppa_basketing where event_id ='" . $raceId . "' and fancier_id ='" . $fancierId . "' and admin_approval = '1'");
        } elseif ($raceId != "" && $fancierId == "") {
            $get_race_details = mysqli_query($dbconnection, "select * from ppa_basketing where event_id ='" . $raceId . "' and admin_approval = '1'");
        }
        ...
        if (mysqli_num_rows($get_race_details) == 0) {
            ...
            while ($race_details = mysqli_fetch_array($get_race_details)) {
                ...
                $data["details"][$i]["bird_name"] = $race_details["bird_name"];
...
case 'scanreader':
    ...
    $userData = mysqli_num_rows($checkuserauth);
    if ($userData > 0) {
        $readerQuery = mysqli_query($dbconnection, "select * from ppa_basketing as a LEFT JOIN ppa_events as b ON a.event_id=b.Events_id LEFT JOIN ppa_bird_type as c ON a.bird_type=c.b_id LEFT JOIN ppa_register as d ON a.fancier_id=d.reg_id
...
case 'racelist':
    ...
    $query = mysqli_query($dbconnection, "SELECT * FROM ppa_events AS A LEFT JOIN users AS B ON A.Org_id=B.users_id LEFT JOIN ppa_event_details AS C ON A.Events_id=C.event_id INNER JOIN ppa_bird_type AS D ON C.bird_id=D.b_id where B.Org_code='" . $apptype . "' GROUP BY A.Events_id DESC");
...
case 'raceeventinfo':
    ...
    if (mysqli_num_rows($raceinfo) > 0) {
        ...
        while ($birdtype_res = mysqli_fetch_array($raceinfo)) {
            ...
            //$cateDate['bird_tag_color'] = $birdtype_res["bird_tag_color"];
...
case 'raceeventinfo_new':
    ...
    $getRaceEvent = mysqli_query($dbconnection, "SELECT b.Events_id, b.Event_name,b.Event_lat,b.Event_long,b.Event_date FROM  ppa_events as b LEFT JOIN users as c on c.users_id = b.Org_id where c.Org_code='" . $apptype . "' and b.Event_date >= '" . $fromdate . "' and b.Event_date <= '" . $todate . "' order by b.Event_date DESC");
...
case 'notification_list':
    ...
    if ($userToken != "") {
        ...
        if (mysqli_num_rows($selectfanciers) > 0) {
            ...
            $getNotification = mysqli_query($dbconnection, "select * from notificationlog where userid='" . $user_id . "'");
