$(document).ready(function() {
    ...
    setTimeout(function() {
      ...
      $status = $obj.attr('title');
  		...
  		$arr = [];
  		$('#statuschange').find('.modal-body').find('input[name="ids"]').remove();
  		...
  		if($arr.length > 0) {
  			//console.log($arr);
        $('#statuschange').find('.yes-btn').attr('href', $base_url+$status+"_"+$arr.join('-'));
  			$('#statuschange').modal('show');
  		}
      else
      {
        $('#selectanyone').modal('show');       
      }
  	});
  ...
  $(".content-wrapper").on("click",".modalClubview", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_clubview',
      method: 'post', 
      data : {
        id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#clubviewwindow').find('.modal-body').html(data);
      $('#clubviewwindow').modal('show'); 
    })
  });

  $(".content-wrapper").on("click",".btnfancierview", function(e) {
    $.ajax({
      url : $('body').attr('data-base-url') + 'user/get_fancierview',
      method: 'post', 
      data : {
        id: $(this).attr('data-src')
      }
    }).done(function(data) {
      $('#fancierviewwindow').find('.modal-body').html(data);
...
function seteventId(id, module) {
  var url =  $('body').attr('data-base-url');
  $("#cnfrm_delete").find("a.yes-btn").attr("href",url+"/"+ module +"/raceeventdelete/"+id);
}

function setfancierId(id, module) {
  var url =  $('body').attr('data-base-url');
  $("#cnfrm_delete").find("a.yes-btn").attr("href",url+"/"+ module +"/fancierdelete/"+id);
}


function clearresult(id, module) {
  var url =  $('body').attr('data-base-url');
  $("#cnfrm_delete").find("a.yes-btn").attr("href",url+"/"+ module +"/clearresult/"+id);
}
...
function pagerefresh(tablide)
{
  $("#"+tablide).DataTable().ajax.reload( null, false );
}
