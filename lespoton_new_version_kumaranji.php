...
// {
//     $action = "Racecalculation Api Hitted before Switch";
...
entrylog("Inside racecalculation api ".$userId,"Inside racecalculation api ".$userId,$userId,$dbconnection);
    ...
    if($deviceTagId != '')
    {
        ...
        $check_tag_id = mysqli_query($dbconnection,"select a.tag_device_id FROM ppa_files as a inner join ppa_events as b on a.event_id = b.Events_id where a.event_id='".$eventId."' and a.tag_device_id='".$deviceTagId."' and b.Event_date <='".$current_date."' order by a.tag_device_id desc limit 0,1");
...
case 'update_user_profile':
    ...
    $version = $postdetails["appVersion"];
    $isupdated = $postdetails["isupdated"];
    ...
    if ($postdetails["variant"] == "open" ) {
                $postdetails["variant"] = '1.5';
            
            }else{
                $postdetails["variant"] = '2.8';
            }

    $appinfo = mysqli_query($dbconnection, "select reg_id from ppa_register where userToken='" . $user_token . "' and reg_id='" . $user_id . "' limit 0,1");

    // "<pre>";print_r($appinfo);

        $checkApp = mysqli_num_rows($appinfo);

    if ($appconfiginfo == 0) {
            $insert_app_config = mysqli_query($dbconnection, "insert appConfig set user_id='" . $user_id . "', variant='" . $variant . "', appVersion='" . $version . "', isupdated='" . $isupdated . "'");
            ...
            $update_app_config = mysqli_query($dbconnection, "update appConfig set user_id='" . $user_id . "', variant='" . $variant . "', appVersion='" . $version . "', isupdated='" . $isupdated . "'");
