...
class Events extends CI_Controller {
    ...
    function sendAnroidPushNotification($message="",$deviceToken="",$pushparameter="",$badge=1,$apptype)
   {
            if($apptype=="PPA")
    ...
    public function update_category()
    {
        ...
        if($res != ""){
            ...
            $query = $this->db->query("select ppa_register.phone_no,ppa_register.apptype,ppa_register.username,ppa_register.android_id from ppa_events inner join users on users.users_id=ppa_events.Org_id inner join ppa_register on ppa_register.apptype=users.Org_code where ppa_events.Events_id='".$eventID."'");
                ...
                foreach ($event_de as $key => $value) {
                    ...
                    $username = $value->username;
                      $message='{"ResponseCode":"100","UserName":"'.$username.'","phone_no":"'.$phone_no.'","Apptype":"'.$apptype.'","Title":"Race Details update ! ","Message":"Admin updated the race details"}';
                      $pushparameter="";
                      $badge=1;
    ...
    public function editrace(){
        ...
        if(CheckPermission("users", "own_read")){
        	...
        	$data['bird_type'] = $this->Event_model->get_data_by($bird_table);
