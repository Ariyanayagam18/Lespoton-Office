...
class User extends CI_Controller {
    ...
    public function dashboard() {
        ...
        if (CheckPermission("users", "own_read")) {
            ...
            $data['livecount'] = $this->User_model->getlivecount();
            $data['upcomingcount'] = $this->User_model->getupcomingcount();
            $data['completedcount'] = $this->User_model->getcompletedcount();
    ...
    public function verifyresult() {
        ...
        if (CheckPermission("users", "own_read")) {
            ...
            $data['result_data'] = $this->User_model->getresults($eventid);
            // echo "string1111111111111111111";
            // echo "<pre>";print_r($data['result_data']);"</pre>";
            // die();
            $data['geteventfancierlists'] = $this->User_model->geteventfancierlists($eventid);
            //echo "string1111111111111111111";
            // echo "<pre>";print_r($data['geteventfancierlists']);"</pre>";
            // die();
            ...
            if (strlen($android_deviceide) == 64) 
                {
                    $pushparameter = "";
                    $badge = 1;
                    $message = "Admin had verified a new photo. You can check result by navigation to result screen.";
                    $title = "Admin verified";
                    $notify_key = 1;
                    $GetMyDeviceId = $android_deviceide;
                    $pushparameter = '{"ResponseCode":"200","badge":"1"}';
                    $this->sendIosPushNotification($title, $message, $GetMyDeviceId,$pushparameter, $badge, $notify_key, $userreg_id, '');
                } 
                else 
                {
                    $pushparameter = "";
                    $badge = 1;
                    $this->sendAnroidPushNotification($message, $android_deviceide, $pushparameter, $badge, $apptype, $notify_key, '');
                }
        } else {
            $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
            redirect(base_url() . 'user/profile', 'refresh');
    ...
    public function removeresultentry() {
        if($_REQUEST["type"] == "1")
        {
            //$update_ppa_fileslats = $this->db->query("update ppa_updatephoto set photo_approval='0' where photo_name ='" . $_REQUEST["img"] . "'");
            $events = $this->db->query("Delete from ppa_report where img_name ='" . $_REQUEST["img"] . "'");
        }else{
            $events = $this->db->query("Delete from ppa_report where img_name ='" . $_REQUEST["img"] . "'");
            //$update_ppa_fileslats = $this->db->query("update ppa_updatephoto set chip_approval='0' where photo_name ='" . $_REQUEST["img"] . "'");
        }
        echo "1";
    }
    public function addresultentry() {
        $events = $this->db->query("SELECT Event_date from ppa_events where Events_id ='" . $_REQUEST["eventide"] . "'");
        // echo "SELECT Event_date from ppa_events where Events_id ='" . $_REQUEST["eventide"] . "'";
        // die();
        ...
        $birdinfo = $this->db->query("SELECT b_id from ppa_bird_type where brid_type ='" . $_REQUEST["birdtype"] . "' and apptype='". $_REQUEST["ctype"] ."'");
        $birdres = $birdinfo->result();

        $bird_prop_info = $this->db->query("SELECT bird_color,bird_gender,owner_ringno,image_status from ppa_files where filename ='" . $_REQUEST["image"] . "' limit 0,1");
        ...
        // echo "SELECT username from ppa_register where apptype ='" . $_REQUEST["ctype"] . "'";
        // die();
        ...
        if (!isset($bird_prop_res[0]->bird_color)) {
            $bird_prop_res[0]->bird_color = '';
        }

        if (!isset($bird_prop_res[0]->bird_gender)) {
            $bird_prop_res[0]->bird_gender = '';
        }
        //echo $_REQUEST["ctype"];die();


        $clubcode = $_REQUEST["ctype"];

        $fancierclubinfo = $this->db->query("SELECT username,apptype,userToken,android_id from ppa_register where apptype ='" . $clubcode . "'");

$fancierinfo = $fancierclubinfo->result();
       //  $get_type = explode(".", $_REQUEST["image"]);
       // if($get_type[1] == "chip")
       // {
       //      $update_ppa_fileslats = $this->db->query("update ppa_updatephoto set chip_approval='1' where photo_name ='" . $_REQUEST["image"] . "'");
       // }

       // if($get_type[1] != "chip")
       // {
       //      $update_ppa_fileslats = $this->db->query("update ppa_updatephoto set photo_approval='1' where photo_name ='" . $_REQUEST["image"] . "'");
       // }
        // echo "<pre>";print_r($fancierclubinfo->result());
        // die();
        //$fancierinfo = $fancierclubinfo->result();
$incc=0;
           /* foreach ($fancierinfo as $key => $user) {
                ...
                $userreg_id = $user->reg_id;
                //$racename = $user->Event_name;               

                $action = "Admin had verified";
                ...
                $title = "Admin had verified a new photo";
                $message = "Admin had verified a new photo. You can check result by navigation to result screen.";
                $pushparameter = "";
                $badge = 1;
                $pushparameter = '{"ResponseCode":"200","badge":"1"}';
                ...
                if (strlen($GetMyDeviceId) == 64) {
                    ...
                    $this->sendAnroidPushNotification($title, $message, $GetMyDeviceId, $pushparameter, $badge, $apptype, $userreg_id, '');

                    print_r($incc." - ".$GetMyDeviceId." <br");
                }
                 $incc++;
            }*/            

        //echo $fancierclubinfo->result();
        ...
        $insertarray["bird_color"] = $bird_prop_res[0]->bird_color;
        $insertarray["bird_gender"] = $bird_prop_res[0]->bird_gender;
        ...
        // $getfancierinfo = $this->db->query("SELECT username from ppa_register where apptype ='" . $_REQUEST["ctype"] . "'");



        echo $ide;

        //echo "<pre>";print_r($getfancierinfo);die();
...
$filepath = base_url();
    ...
    public function viewbasketing() {
        is_login();
        //echo "<pre>";print_r($eventide = $this->uri->segment(3));
         //echo "<pre>";print_r($this->session->userdata('user_details')[0]->user_type);
        // die();
        if($this->session->userdata('user_details')[0]->user_type == 'admin' || $this->session->userdata('user_details')[0]->user_type == 'Admin')
            {
                $this->load->view('include/header');
                $eventide = $this->uri->segment(3);
                $eventide = base64_decode(urldecode($eventide));
                //echo "<pre>"; print_r($eventide); die;
                //$user_id = $this->session->userdata('user_details')[0];
                $data['result_data'] = $this->User_model->getresults($eventide);
                //echo "<pre>"; print_r($data); print_r($user_id); die;
                $data['racebirdtypes'] = $this->User_model->getbirdtypesbyrace($eventide);
                $data['geteventfancierlists'] = $this->User_model->geteventfancierlists($eventide);
                $this->load->view('view_basketentry', $data);
                $this->load->view('include/footer');
            }elseif($this->session->userdata('user_details')[0]->user_type == 'member' || $this->session->userdata('user_details')[0]->user_type == 'Member') {
                $this->load->view('include/header');
                $eventide = $this->uri->segment(3);
                $eventide = base64_decode(urldecode($eventide));
                $get_event_id = $this->db->query("select Org_id from ppa_events where Events_id = '".$eventide."'");
                $get_event_id = $get_event_id->row_array();
                $users_id = $this->session->userdata("user_details")[0]->users_id;
                if($users_id == $get_event_id['Org_id'])
                {
                    ...
                    $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
                    redirect(base_url() . 'user/error_page', 'refresh');
                }     
            }else{
                $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
                redirect(base_url() . 'user/profile', 'refresh');
    ...
    public function raceresults() {
        ...
        if (CheckPermission("users", "own_read")) {
            ...
            $resultid = base64_decode(urldecode($resultid));
    ...
    public function changeaccountstatus() {
        $status = $_POST["status"];
        $ide = $_POST["ide"];
        $fancier_info = $this->db->query("SELECT android_id, username, reg_id, phone_no, apptype from ppa_register where reg_id ='" . $_POST["ide"] . "' limit 0,1");

        $fancier_info_res = $fancier_info->result();
        $android_deviceide = $fancier_info_res[0]->android_id;
        $fancier_name = $fancier_info_res[0]->username;
        $userreg_id = $fancier_info_res[0]->reg_id;
        $phone_no = $fancier_info_res[0]->phone_no;
        $apptype = $fancier_info_res[0]->apptype;
        ...
        if ($status == "0") {
            ...
            $message = "Dear " . $fancier_name . " " . " your account is activated";
            $body = "Dear " . $fancier_name . " " . " your account is activated";
            $pushparameter = "";
            $badge = 1;
            $pushparameter = '{"ResponseCode":"200","badge":"1"}';
    ...
    public function changetagstatus() {
        ...
        if ($status == "0") {
            $action = "Inactived the tag details for the fancier";
            $desc = $this->session->userdata('user_details')[0]->name . " Inactived the tag details for the fancier Mr." . $fancier_name;
            $this->User_model->entrylog($action, $desc);
            $query = $this->db->query("update ppa_register set bluetooth_id='',deviceName='',macAddress='',deviceYear='' where reg_id='" . $ide . "'");
            $this->db->where('user_id', $ide);
            $delete = $this->db->delete('ppa_tag_details');
        } else {
            $action = "Actived the tag details for the fancier";
            $desc = $this->session->userdata('user_details')[0]->name . " Actived the tag details for the fancier Mr." . $fancier_name;
            $this->User_model->entrylog($action, $desc);
            //$query = $this->db->query("update ppa_register set status='" . $status . "' where reg_id='" . $ide . "'");
        }
        // echo "update ppa_register set status='".$status."' where reg_id='".$ide."'";
    ...
    public function changeadminstatus() {
        ...
        $query = $this->db->query("update ppa_register set usertype='" . $status . "',userToken='' where reg_id='" . $ide . "'");
        // echo "update ppa_register set status='".$status."' where reg_id='".$ide."'";
        echo "0";
    ...
    public function changeloftstatus() {
        $status = $_POST["status"];
        $ide = $_POST["ide"];
        $fancier_info = $this->db->query("SELECT android_id,username,loft_image,apptype,reg_id,phone_no from ppa_register where reg_id ='" . $_POST["ide"] . "' limit 0,1");
        $fancier_info_res = $fancier_info->result();
        //echo "<pre>"; print_r($fancier_info_res[0]->loft_image); die;
        $fancier_name = $fancier_info_res[0]->username;
        $android_deviceide = $fancier_info_res[0]->android_id;
        $userreg_id = $fancier_info_res[0]->reg_id;
        $apptype = $fancier_info_res[0]->apptype;
        ...
        if ($fancier_info_res[0]->loft_image != "") 
        {
            if ($status == 0) 
            {
            ...
            else 
            {
                ...
                $title = "Your loft is activated.";
                $message = "Dear " . $fancier_name . " " . " your loft is activated";
                $body = "Dear " . $fancier_name . " " . " your loft is activated";
                $pushparameter = "";
                $badge = 1;
                $pushparameter = '{"ResponseCode":"200","badge":"1"}';
    ...
    public function changebasketstatus() 
    {
        $ide = $_POST["ide"];
        $fanide = $_POST["fanide"];
        $birdType = $_POST["bird_type"];

        $getCheckReaderId = $this->db->query("SELECT scanReader,event_id from ppa_basketing where entry_id ='" . $ide . "' limit 0,1");
        $CheckReaderId = $getCheckReaderId->row_array();

        $get_read_type = $this->db->query("SELECT read_type from ppa_register where reg_id ='" . $fanide . "' limit 0,1");
        $get_read_type = $get_read_type->row_array();

        $race_id = $CheckReaderId['event_id'];
        $get_date_time = $this->db->query("select * from ppa_events as eve LEFT JOIN ppa_event_details as eve_b ON eve.Events_id = eve_b.event_id where eve_b.event_id = '" . $race_id . "' and eve_b.bird_id = '". $birdType ."' order by eve_b.event_id DESC limit 0,1");
        $get_date_time = $get_date_time->row_array();
        
        if(count($get_date_time) > 0){
            $concate_start_date_time = $get_date_time['date'] . ' ' . $get_date_time['start_time'];
            $concate_start_date_time = strtotime($concate_start_date_time);
            $concate_end_date_time = $get_date_time['date'] . ' ' . $get_date_time['end_time'];
            $concate_end_date_time = strtotime($concate_end_date_time);
            //echo "<pre>"; print_r($concate_start_date_time); print_r($concate_end_date_time); die;
            $curDate = date("Y-m-d h:i:s A");
            if ($concate_start_date_time <= strtotime($curDate) && $concate_end_date_time >= strtotime($curDate)) {
                $data["status"] = 404;
                $data["message"] = "You can't basket bird now since the race is live or completed.";
                echo "date_exp";
                die;
                //$cateDate['race_status'] = "LIVE";
            } else if($concate_end_date_time < strtotime($curDate)) {
                $data["status"] = 404;
                $data["message"] = "You can't basket bird now since the race is live or completed.";
                echo "date_exp";
                die;
        ...
        if($get_read_type['read_type'] == "0")
        {
            ...
            foreach ($selectuserlist as $key => $user) 
            {
                ...
                $apptype = $user->apptype;
                $phone_no = $user->phone_no;
                $racename = $get_date_time['Event_name'];
                $userreg_id = $user->reg_id;
                ...
                if (strlen($android_deviceide) == 64) 
                {
                    ...
                    $badge = 1;
                    $notify_key = 1;
                    $message = 'Hello Mr.' . $username . ', ' . " we approved the your basketing entry Race name : " . $racename;
                    $body = 'Hello Mr.' . $username . ', ' . " we approved the your basketing entry Race name : " . $racename;
                    $this->sendIosPushNotification($title, $body, $message, $android_deviceide, $pushparameter, $badge, $notify_key, $userreg_id, $apptype, $phone_no, $action);
                } 
                else 
                {
    ...
    public function changeresultstatus() {
        ...
        if ($status == "0") {
            ...
            if($ide != "")
            {
                $selectfanciers = $this->db->query("SELECT a.*,c.Event_name FROM `ppa_register` as a LEFT JOIN users as b ON a.`apptype` = b.Org_code LEFT JOIN ppa_events as c ON c.Org_id= b.users_id WHERE c.Events_id='" . $ide . "'");
                $selectuserlist = $selectfanciers->result();
                foreach ($selectuserlist as $key => $user) {
                    $android_deviceide = $user->android_id;
                    $username = $user->username;
                    ...
                    $title = 'Hello Mr.' . $username . ', ' . $racename . ' race result released please check out.';
                    $body = 'Race result released please check out.';
                    $badge = 1;
                    if (strlen($android_deviceide) == 64) 
                    {
                        $pushparameter = "";
    ...
    public function clearresult($id) {
        ...
        //$update_ppa_fileslats = $this->db->query("update ppa_updatephoto set photo_approval='0',chip_approval='0' where event_id ='" . $id . "'");
        $this->session->set_flashdata('messagePr', 'Results cleared successfully');
        $encrypt = urlencode(base64_encode($id));
        redirect(base_url() . 'user/raceresults/' . $encrypt, 'refresh');
    }
    ...
    public function updateentryresult() {
        ...
        for ($ide = 0; $ide < count($entryid); $ide++) {
            ...
            $query = $this->db->query("select scanReader from ppa_basketing where entry_id ='" . $entryide . "'");
            $get_basket_details = $query->row_array();

            if($get_basket_details['scanReader'] != "")
            {
                $data["admin_approval"] = "1";
            }
    ...
    public function updatebasket() {
        ...
        for ($ide = 0; $ide < count($ringno); $ide++) {
            ...
            foreach ($selectUserList as $key => $user) {
                ...
                $desc = $this->session->userdata('user_details')[0]->name . "  approved the fancier " . $username . " basketing entry ringno : " . $bird_ringno;
                $this->User_model->entrylog($action, $desc);
                // $message = '{"ResponseCode":"100","UserName":"' . $username . '","phone_no":"' . $phone_no . '","Apptype":"' . $apptype . '","Title":"Race Entry","Message": "' . $username . '"modified the Race}';
                // echo "<pre>Message111:";

                $title = "New Bird Entry";
                $message = "Dear " . $username . " " . "club ring no:" . $bird_ringno . " has been added to the race";
                $body = "Dear " . $username . " " . "club ring no:" . $bird_ringno . " has been added to the race";
                $pushparameter = "";
                $badge = 1;
                $pushparameter = '{"ResponseCode":"200","badge":"1"}';
    ...
    public function updatelatlong() {
        ...
        if($password != "")
        {
            $query = $this->db->query("update ppa_register set latitude='" . $lat . "',longitude='" . $long . "',read_type='" . $read_type . "' ,android_id='" . $device_id . "',password='". $hashPassword ."' where reg_id='" . $ide . "'");
        }else{
            $query = $this->db->query("update ppa_register set latitude='" . $lat . "',longitude='" . $long . "',read_type='" . $read_type . "' ,android_id='" . $device_id . "' where reg_id='" . $ide . "'"); 
        }
        $this->session->set_flashdata('messagePr', 'Fancier details updated Successfully');
    ...
    public function deletebasketentry() {
        ...
        $select = $this->db->query("select entry_id from pigeons where entry_id = '" . $ide . "'");
        $select = $select->row_array();
        if(isset($select["entry_id"]))
        {
            $delete = $this->db->query("delete from pigeons where entry_id='" . $ide . "'");
        }
    }
    /**
    ...
    public function addappadmin() {
        ...
        $check_exist = $this->db->query("select reg_id from ppa_register where apptype='" . $_POST["clubname"] . "' and phone_no='" . $_POST["contact_number"] . "' and email_id='" . $_POST["email_id"] . "'");
        ...
        if ($checkcount == 0) {
            ...
            $this->session->set_flashdata('messagePr', '<span style="color:yellow;">Mentioned ( ' . $_POST["contact_number"] . ' || ' . $_POST["email_id"] . ' ) contact number or email id already added as fancier.. Please check !!</span> ');
            redirect(base_url() . 'user/fancier', 'refresh');
        }
        die;
    }
    ...
    public function systemTable() {
        ...
        //echo "<pre>"; print_r(SSP::complex($_GET, $sql_details, $table, $primaryKey, $columns, $Join_condition, $where, $groupby)); die;
    ...
    public function fancierTable() {
        ...
        $primaryKey = 'reg_id';
        $columns = array(
            array('db' => 'ppa_a.reg_id', 'dt' => 0, 'field' => 'reg_id'),
            array('db' => 'ppa_a.reg_id', 'dt' => 1, 'field' => 'reg_id'),
            array('db' => 'ppa_a.username', 'dt' => 2, 'field' => 'username'),
            array('db' => 'ppa_a.phone_no', 'dt' => 3, 'field' => 'phone_no'),
            array('db' => 'ppa_a.address', 'dt' => 4, 'field' => 'address'),
            array('db' => 'ppa_a.status', 'dt' => 5, 'field' => 'status'),
            array('db' => 'ppa_a.loftstatus', 'dt' => 6, 'field' => 'loftstatus'),
            array('db' => 'ppa_a.usertype', 'dt' => 7, 'field' => 'usertype'),
            array('db' => 'ppa_a.loft_image', 'dt' => 8, 'field' => 'loft_image'),
            array('db' => 'ppa_a.deviceName', 'dt' => 9, 'field' => 'deviceName'),
            array('db' => 'ppa_a.read_type', 'dt' => 10, 'field' => 'read_type'),
            array('db' => 'ppa_a.reg_id', 'dt' => 11, 'field' => 'reg_id'),
        );

        // $columns = array(
        //     array('db' => 'ppa_a.reg_id', 'dt' => 0, 'field' => 'reg_id'),
        //     array('db' => 'ppa_b.user_id', 'dt' => 0, 'field' => 'user_id'),
        // );
        ...
        $Join_condition = " from ppa_register as ppa_a LEFT JOIN ppa_tag_details as ppa_b ON ppa_b.user_id = ppa_a.reg_id ";

        $clubtype = $this->session->userdata('user_details')[0]->Org_code;

        if ($this->session->userdata('user_details')[0]->user_type != 'Admin') {
            $where = "ppa_a.apptype = '" . $clubtype . "' and ppa_a.apptype != ''";
        } else {
            if (isset($_GET["companytype"]) && $_GET["companytype"] != '') {
                $where = "ppa_a.apptype = '" . $_GET["companytype"] . "'";
                //$where = array("ppa_a.apptype = '" . $_GET["companytype"] . "'");
            } else {
                $where = "";
            }

        }
        //$orderby = "order by ppa_a.username ASC";
        $groupby = "";
        //echo "SS ".$where; die;
        //echo "<pre>"; print_r(SSP::complex($_GET, $sql_details, $table, $primaryKey, $columns, $Join_condition, $where, $groupby)); die;
        $output_arr = SSP::simple($_GET, $sql_details, $table, $primaryKey, $columns, $Join_condition, $where, $groupby);
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            if (trim($output_arr['data'][$key][5]) == '1') {
                ...
                if (trim($output_arr['data'][$key][7]) == '1') {
                    $output_arr['data'][$key][7] = '<a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',0);" title="click to make fancier">Fancier</a> | <a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',2);" title="click to make admin">Liberation</a> | <span style="color:blue;font-weight:bold;">Basketing</span>';
                } else if(trim($output_arr['data'][$key][7]) == '2'){
                    $output_arr['data'][$key][7] = '<a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',0);" title="click to make admin">Fancier</a> | <span style="color:blue;font-weight:bold;">Liberation</span> | <a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',1);" title="click to make liberation">Basketing</a>';
                }else if(trim($output_arr['data'][$key][7]) == '0'){
                    $output_arr['data'][$key][7] = '<span style="color:blue;font-weight:bold;">Fancier</span> | <a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',2);" title="click to make admin">Liberation</a> | <a style="cursor:pointer;" onclick="changeadminstatus(' . $id . ',1);" title="click to make admin">Basketing</a>';
                }
            ...
            if(trim($output_arr['data'][$key][10]) == '1' && $output_arr['data'][$key][9] != "") {
                $output_arr['data'][$key][10] = '<span style="color:green;">Active</span> | <a style="cursor:pointer;"  onclick="changetagstatus(' . $id . ',0);">Make Inactive</a>';
            }else{
               $output_arr['data'][$key][10] = ''; 
            }
    ...
    public function viewbasketingTable() {
        ...
        $birdtype = $this->db->query("SELECT DISTINCT ppa_bird_type.b_id,ppa_bird_type.brid_type ,ed_id, race_distance , event_id  FROM `ppa_event_details` INNER JOIN ppa_bird_type ON ppa_event_details.bird_id=ppa_bird_type.b_id where ppa_event_details.event_id='" . $raceeventide . "'");
        // echo "SELECT DISTINCT ppa_bird_type.b_id,ppa_bird_type.brid_type ,ed_id, race_distance , event_id  FROM `ppa_event_details` INNER JOIN ppa_bird_type ON ppa_event_details.bird_id=ppa_bird_type.b_id where ppa_event_details.event_id='" . $raceeventide . "'";die;

        // SELECT DISTINCT ppa_bird_type.b_id,ppa_bird_type.brid_type ,ed_id, race_distance , event_id FROM `ppa_event_details` INNER JOIN ppa_bird_type ON ppa_event_details.bird_id=ppa_bird_type.b_id where ppa_event_details.event_id='1091'
        ...
        $columns = array(
            ...
            array('db' => 'a.color', 'dt' => 5, 'field' => 'color'),
            array('db' => 'a.rubber_outer_no', 'dt' => 6, 'field' => 'rubber_outer_no'),
            array('db' => 'a.scanReader', 'dt' => 7, 'field' => 'scanReader'),
            array('db' => 'b.bluetooth_id', 'dt' => 8, 'field' => 'bluetooth_id'),
            array('db' => 'b.bluetooth_id', 'dt' => 9, 'field' => 'bluetooth_id'),
            array('db' => 'b.bluetooth_id', 'dt' => 10, 'field' => 'bluetooth_id'),

            array('db' => 'a.rubber_inner_no', 'dt' => 11, 'field' => 'rubber_inner_no'),
            array('db' => 'a.sticker_outer_no', 'dt' => 12, 'field' => 'sticker_outer_no'),
            array('db' => 'a.sticker_inner_no', 'dt' => 13, 'field' => 'sticker_inner_no'),
            //array( 'db' => 'a.admin_approval', 'dt' => 10 ,'field' => 'admin_approval'),
            array('db' => 'CONCAT(a.admin_approval,"#",a.fancier_id,"#",a.bird_type)', 'dt' => 14, 'field' => 'CONCAT(a.admin_approval,"#",a.fancier_id,"#",a.bird_type)'),
            array('db' => 'a.entry_id', 'dt' => 15, 'field' => 'entry_id'),
        );
        //array( 'db' => 'a.entry_id', 'dt' => 11 ,'field' => 'entry_id')
        $sql_details = array(
            'user' => $this->db->username,
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            if ($output_arr['data'][$key][8]) {
                $devicename = json_decode($output_arr['data'][$key][8]);
                $output_arr['data'][$key][8] = $devicename->deviceName;
            }

            if ($output_arr['data'][$key][9]) {
                $macAddress = json_decode($output_arr['data'][$key][9]);
                $output_arr['data'][$key][9] = $macAddress->macAddress;
            }

            if ($output_arr['data'][$key][10]) {
                $deviceYear = json_decode($output_arr['data'][$key][10]);
                $output_arr['data'][$key][10] = $deviceYear->deviceYear;
            }
            ...
            $output_arr['data'][$key][11] = $output_arr['data'][$key][11] . "<input class='form-control' style=width:120px; type='hidden' name='rubber_inner_no[]' value='" . $output_arr['data'][$key][11] . "'>";
            $output_arr['data'][$key][12] = $output_arr['data'][$key][12] . "<input class='form-control' style=width:120px; type='hidden' name='sticker_outer_no[]' value='" . $output_arr['data'][$key][12] . "'>";
            $output_arr['data'][$key][13] = $output_arr['data'][$key][13] . "<input class='form-control' style=width:120px; type='hidden' name='sticker_inner_no[]' value='" . $output_arr['data'][$key][13] . "'>";

            $adminapprovalinfo = explode('#', $output_arr['data'][$key][14]);

            if (trim($adminapprovalinfo[0]) == '0') {
                $output_arr['data'][$key][14] = '<span style="color:red;">Not approved</span> | <a style="cursor:pointer;" onclick="changebasketstatus(
                ' . $id . ',' . $adminapprovalinfo[1] . ','. $adminapprovalinfo[2] .');">Approve now</a>';
            } else {
                $output_arr['data'][$key][14] = '<span style="color:green;">Approved</span> ';
            }
    ...
    public function viewresultssummary() {
        is_login();
        $raceeventide = $_GET["event"];
        // echo "<pre>"; print_r($raceeventide); die;
        $table = 'ppa_files';
        $primaryKey = 'ide';
        if (isset($_GET["editmode"]) && $_GET["editmode"] != 0) {
            $editmode = 1;
        ...
        $post_date = str_replace("'", '', $racestartdate) . " " . $starttime; //Event Date with Time
        ...
        // $verified_photo = $this->db->query("SELECT photo_name FROM ppa_updatephoto WHERE event_id ='" . $raceeventide . "' AND club_code='" . $clubcodeinfo . "' AND photo_approval='1'");
        // echo "SELECT photo_name FROM ppa_updatephoto WHERE event_id ='" . $raceeventide . "' AND club_code='" . $clubcodeinfo . "'"; die;
        $verified_photores = $verified_photo->result();
        foreach ($verified_photores as $verified_photorow) {
            $approved_photos[] = $verified_photorow->photo_name;
        }

        // Added this if else condition because different user has same interval so don't delete this if else condition
        if($raceeventide != "")
        {
            $verified = $this->db->query("SELECT intervel,img_name FROM ppa_report WHERE apptype='" . $clubcodeinfo . "' and event_id='".$raceeventide."'" );
        }else{
            $verified = $this->db->query("SELECT intervel,img_name FROM ppa_report WHERE apptype='" . $clubcodeinfo . "'" ); 
        }

        $verifieddata = $verified->result();
        $intervel = array();

        // Hide by kumaran because same interval occures for different user thats the problem
        // foreach ($verifieddata as $verifyrow) {
        //     $intervel[] = $verifyrow->intervel;
        // }
        foreach ($verifieddata as $verifyrow) {
            $intervel[] = $verifyrow->img_name;
        }
        ...
        if ($photo_typefilter == 'Approved') {
            ...
            //$where = " " . $fanciercond . "latitude!='' and a.filename not like '%liberation%' and DATE_FORMAT(a.cre_date,'%Y-%m-%d','%Y-%m-%d') <=" . $startdate . " and DATE_FORMAT(a.cre_date,'%Y-%m-%d','%Y-%m-%d') >=" . $enddate . " and a.club_code=" . $clubcode . " and d.birdtype = " . $bird_ides . " and d.event_id=" . $raceeventide;

            $where = " " . $fanciercond . "latitude!='' and a.filename not like '%liberation%' and a.club_code=" . $clubcode . " and d.birdtype = " . $bird_ides . " and a.event_id=" . $raceeventide."";
            ...
            $columns = array(
                ...
                array('db' => 'a.velocity', 'dt' => 8, 'field' => 'velocity'),
                ...
                array('db' => 'a.tag_device_id', 'dt' => 14, 'field' => 'tag_device_id'),
                array('db' => 'a.ide', 'dt' => 15, 'field' => 'ide'),

            );
        } else {
            $Join_condition = ' from ppa_files as a';
            //$where = " " . $fanciercond . "latitude!='' and a.filename not like '%liberation%' and DATE_FORMAT(a.cre_date,'%Y-%m-%d') <=" . $startdate . " and DATE_FORMAT(a.cre_date,'%Y-%m-%d') >=" . $enddate . " and a.club_code=" . $clubcode;
            ...
            if($raceeventide != "")
            {
                $where = " " . $fanciercond . "latitude!='' and a.filename not like '%liberation%' and a.event_id = ". $raceeventide ." and bird_type = ".$bird_ides." and a.club_code=" . $clubcode;
            }
            else{
                $where = " " . $fanciercond . "latitude!='' and a.filename not like '%liberation%' and DATE_FORMAT(a.cre_date,'%Y-%m-%d') <=" . $startdate . " and DATE_FORMAT(a.cre_date,'%Y-%m-%d') >=" . $enddate . " and a.club_code=" . $clubcode;
            }
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            //postdate =>  Event Date with Time
            //fulltime => current time with date
            if ($clickeddate == $starteddate) { // Current Date and race date comparing
                $starttime = strtotime($post_date); // Event Date with time
                ...
                foreach ($chk_datedata as $chk_datedatarow) {
                    ...
                    if ($clickeddate == $racestartdate) {
                        ...
                        //echo h2m("1:45");
                        $phour = $phour + $this->User_model->h2m($chk_datedatarow->boarding_time); // convert Hours to minutes
                    }
            ...
            if ($output_arr['data'][$key][8] == "") // update velocity if not update.
            {
                $velocity_update = $this->db->query("update ppa_files set velocity='" . $velocity1 . "' where ide='" . $ide . "'");
            }

            if($output_arr['data'][$key][8] != $velocity1)
            {
                $velocity_update = $this->db->query("update ppa_files set velocity='" . $velocity1 . "' where ide='" . $ide . "'");
            }

            if($output_arr['data'][$key][7] != $distance123)
            {
                $distance_update = $this->db->query("update ppa_files set distance='" . $distance123 . "' where ide='" . $ide . "'");
            }

            $output_arr['data'][$key][6] = $calculationhour;//number_format($calculationhour, 7); // Time Taken
            $output_arr['data'][$key][8] = number_format($velocity1, 7);
            $output_arr['data'][$key][7] = $distance123 . " Km";//number_format(($distance123*1000), 7) . " Metres";
            ...
            $photos_name = $output_arr['data'][$key][3];
            $getphotoapproval = $this->db->query("select photo_approval,chip_approval from ppa_updatephoto where photo_name = '".$photos_name."'");
            $photoapproval = $getphotoapproval->row_array();

            //if (($output_arr['data'][$key][14] != "" && $photoapproval['chip_approval'] == "1") || ($photoapproval['photo_approval'] == "1")) {
            if (in_array($photos_name, $intervel)) {
                $approve = '<font style=color:red;>Verified</font><br><input class="btn btn-default btn-active" style="background:red;color: #fff;border-color: red;" type="button" value="Revert Back" onclick="revertback(\'' . $output_arr['data'][$key][3] . '\',' . $raceeventide . ',1)">';
            ...
            $users_ids  = $this->session->userdata('user_details')[0]->users_id;
            if($users_ids == "1")
            {
                $output_arr['data'][$key][15] = '<input type="button" value="Delete" onclick="delete_report(' . $output_arr['data'][$key][0] . ')" class="btn btn-default">';
            }
            ...
            $approved_html = "";
            if (isset($output_arr['data'][$key][3]) && !empty($output_arr['data'][$key][3])) {
                $photo_name = $output_arr['data'][$key][3];
                $get_photo_approval = $this->db->query("select photo_approval from ppa_updatephoto where photo_name = '".$photo_name."'");
                $photo_approval = $get_photo_approval->row_array();
                if($photo_approval['photo_approval'] == "1" || $photo_approval['photo_approval'] == 1)
                {
                    $approved_html = '';
                    $approved_html = '<i style="font-size:25px;" title="Fancier approved photo" class="fa fa-check" aria-hidden="true"></i>';
                }else{
            ...
            if (@getimagesize($filename) || $photoapproval['chip_approval'] == "0") {
                $chip_extension = explode('.', $filename);
                if($chip_extension[3] == "chip" || $chip_extension[2] == "chip")
                {
                ...
                {
                    $output_arr['data'][$key][3] = 'No image';
                    $approve = '<font style=color:red;>Verified</font><br><input class="btn btn-default btn-active" style="background:red;color: #fff;border-color: red;" type="button" value="Revert Back" onclick="revertback(\'' . $photos_name . '\',' . $raceeventide . ',2)">';
                }
            }
    ...
    public function delete_ppa_files()
    {
        $this->db->where('ide', $_POST['id']);
        $this->db->delete('ppa_files');
        return 1;
    }
    ...
    public function stickercolors() {
        is_login();
        if (CheckPermission("users", "own_read")) {
            $this->load->view('include/header', $data);
            $this->load->view('stickercolors', $data);
            $this->load->view('include/footer');
        } else {
            $this->session->set_flashdata('messagePr', 'You don\'t have permission to access.');
            redirect(base_url() . 'user/profile', 'refresh');
    ...
    public function stickercoloraddedit() {
        ...
        if ($ide > 0) // Edit
        {
            $update_qry = $this->db->query("update pigeon_sticker_colors set sticker_color='" . $colorname . "',apptype='". $apptype ."',user_id='". $user_id ."' where ide='" . $ide . "'");
            echo "0";
        } else // Add
        {
            $get_sticker_name = $this->db->query("select sticker_color from pigeon_sticker_colors where sticker_color = '". $colorname ."' and apptype='". $apptype ."'");
            $sticker_name = $get_sticker_name->row_array();

            if(!empty($sticker_name))
            {
                echo "dup";
            }else{
                $insert_qry = $this->db->query("insert into pigeon_sticker_colors set sticker_color='" . $colorname . "',apptype='". $apptype ."',user_id='". $user_id ."'");
                echo "0";
            }
        }
    }
    ...
    public function coloraddedit() {
        ...
        if ($ide > 0) // Edit
        {
            $update_qry = $this->db->query("update pigeons_color set color='" . $colorname . "',apptype='". $apptype ."',user_id='". $user_id ."' where ide='" . $ide . "'");
        } else // Add
        {
            $insert_qry = $this->db->query("insert into pigeons_color set color='" . $colorname . "',apptype='". $apptype ."',user_id='". $user_id ."'");
        }
        echo "0";
    }
    public function colorTable() {
        ...
        //$Join_condition = " from pigeons_color";
    ...
    public function stickerColor() {
        is_login();
        $table = 'pigeon_sticker_colors';
        $primaryKey = 'ide';
        $columns = array(
            array('db' => 'ide', 'dt' => 0),
            array('db' => 'sticker_color', 'dt' => 1),
            array('db' => 'ide', 'dt' => 2),
        );
    ...
    public function birdaddedit() {
        ...
        if ($ide > 0) // Edit
        {
            $update_qry = $this->db->query("update ppa_bird_type set brid_type='" . $birdname . "',apptype='". $apptype ."',user_id='". $user_id ."' where b_id='" . $ide . "'");
        } else // Add
        {
            $insert_qry = $this->db->query("insert into ppa_bird_type set brid_type='" . $birdname . "',bird_status='1',apptype='". $apptype ."',user_id='". $user_id ."'");
        }
        echo "0";
    }
    ...
    public function resultsTable() {
        ...
        $columns = array(
            ...
            array('db' => 'e.brid_type', 'dt' => 4, 'field' => 'brid_type'),
            array('db' => 'c.ring_no', 'dt' => 5, 'field' => 'ring_no'),
            array('db' => 'c.img_name', 'dt' => 6, 'field' => 'img_name'),
            array('db' => 'c.bird_name', 'dt' => 7, 'field' => 'bird_name'),
            array('db' => 'c.bird_gender', 'dt' => 8, 'field' => 'bird_gender'),
            array('db' => 'c.bird_color', 'dt' => 9, 'field' => 'bird_color'),

            array('db' => 'c.outer_no', 'dt' => 10, 'field' => 'outer_no'),
            //array( 'db' => 'c.start_date', 'dt' => 8,'field' => 'start_date'),
            array('db' => 'c.start_time', 'dt' => 11, 'field' => 'start_time'),
            array('db' => 'c.intervel', 'dt' => 12, 'field' => 'intervel'),
            array('db' => 'c.intervel', 'dt' => 13, 'field' => 'intervel'),
            array('db' => 'c.minis', 'dt' => 14, 'field' => 'minis'),
            array('db' => 'c.distance', 'dt' => 15, 'field' => 'distance'),
            array('db' => 'c.velocity', 'dt' => 16, 'field' => 'velocity'),
            array('db' => 'c.report_id', 'dt' => 17, 'field' => 'report_id'),
            //array('db' => 'pb.scanReader', 'dt' => 16, 'field' => 'scanReader'),
            //array( 'db' => 'c.latitude', 'dt' => 15,'field' => 'latitude'),
            //array( 'db' => 'c.longtitude', 'dt' => 16,'field' => 'longtitude')
        );
        ...
        $clubtype = $this->session->userdata('user_details')[0]->Org_code;
        $Join_condition = ' from ppa_report as c LEFT JOIN users as d ON d.users_id = c.clubtype LEFT JOIN ppa_bird_type as e ON e.b_id=c.bird_type_id LEFT JOIN ppa_files as pb ON pb.event_id = c.event_id';
        // if(trim($clubtype)!='' && $this->session->userdata('user_details')[0]->user_type !='Admin')
        if ($birdtype != '') {
            $where = "c.event_id = '" . $raceeventide . "' and c.bird_type_id = '" . $birdtype . "'";
        } else {
        ...
        foreach ($output_arr['data'] as $key => $value) {
             ...
             */
            $files = $output_arr['data'][$key][6];
            $get_tag_number = $this->db->query("select tag_device_id from ppa_files where filename = '".$files."'");
            $tag_id = $get_tag_number->row_array();
            $output_arr['data'][$key][6] = $tag_id['tag_device_id'];
            $curDate = date("Y-m-d h:i:s A");
            $output_arr['data'][$key][12] = date('Y-m-d', ($output_arr['data'][$key][12] / 1000));
            $output_arr['data'][$key][13] = date('h:i:s A', ($output_arr['data'][$key][13] / 1000));

            if ($editmode == 1) {
                $output_arr['data'][$key][5] = "<input class='form-control' style=width:120px; type='textbox' name='ringno[]' onkeyup=getinfo(this.value,'" . $rows . "') value='" . $output_arr['data'][$key][5] . "'>";
                $output_arr['data'][$key][7] = "<input class='form-control' style=width:120px; type='textbox' name='birdname[]' id='birdname" . $rows . "' value='" . $output_arr['data'][$key][7] . "'>";

                $birdcolordropdown = "<select name=birdcolor[] id=colortype" . $rows . " class=form-control input-sm>";
                $colorhightlight = '';
                for ($t = 0; $t < count($birdcolor); $t++) {
                    if ($output_arr['data'][$key][9] == $birdcolor[$t]) {
                        $colorhightlight = "selected";
                    } else {
                        $colorhightlight = "";
                    }
                ...
                $genderhightlight = '';
                for ($t = 0; $t < count($genderarray); $t++) {
                    if ($output_arr['data'][$key][8] == $genderarray[$t]) {
                        $genderhightlight = "selected";
                    } else {
                        $genderhightlight = "";
                    }
                ...
                $output_arr['data'][$key][8] = $genderdropdown;
                $output_arr['data'][$key][9] = $birdcolordropdown;
            ...
            $output_arr['data'][$key][17] = '<input type="button" value="Delete" onclick="delete_report(' . $output_arr['data'][$key][0] . ')" class="btn btn-default">';
    ...
    public function deletereport() {
        ...
        $old_data_results = $this->db->query("select * from ppa_report as c where c.report_id='" . $rid . "'");
        $old_data_resultsres = $old_data_results->result();
        // $select = $this->db->query("select * from ppa_updatephoto where photo_name='" . $old_data_resultsres[0]->img_name . "'");
        ...
        $this->User_model->entrylog($action, $desc);
        //$update_qry = $this->db->query("update ppa_updatephoto set photo_approval='0' where photo_name='" . $old_data_resultsres[0]->img_name . "'");
    ...
    public function racesTable() {
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            //echo "<pre>"; print_r($concate_start_date_time); print_r($concate_end_date_time); 
            $concate_end_date_time = strtotime($concate_end_date_time);
            //echo "<pre>"; print_r($concate_start_date_time); print_r(" ".$curdate); print_r(" ".$concate_end_date_time); die;
            $encrypt = urlencode(base64_encode($id));
            if (trim($output_arr['data'][$key][7]) != '' && $output_arr['data'][$key][7] > 0) // Race completed wont delete and edit
            {
                ...
                //
                $output_arr['data'][$key][count($output_arr['data'][$key]) - 1] .= '<a id="btnEditRow" class="modalButtonUser mClass"  href="' . base_url() . 'events/editrace/' .  $encrypt . '"  type="button" data-src="' . $id . '" title="Edit"><i class="fa fa-pencil" data-id=""></i></a><a data-toggle="modal" class="mClass" onclick="seteventId(' . $id . ', \'user\')"   data-src="' . $id . '" data-target="#cnfrm_delete" title="Delete race"><i class="fa fa-trash" data-id=""></i></a>';
                ...
                } else if ($concate_start_date_time <= $curdate && $concate_end_date_time >= $curdate) {
                    $output_arr['data'][$key][7] = "<span style=color:green;font-weight:bold;>LIVE</span>";
                    $output_arr['data'][$key][8] = '<span style=color:green;font-weight:bold;><a href="' . base_url() . 'user/racesummary/' . $encrypt . '">Live Results</a></span>';
                    $output_arr['data'][$key][10] = '<span style=color:green;font-weight:bold;><a href="' . base_url() . 'user/verifyresult/' . $encrypt . '">Verify Results</a></span>';
                } // raceinfo racesummary
                else {
                    $output_arr['data'][$key][7] = "<span style=color:blue;font-weight:bold;>Completed</span><br><a href='" . base_url() . 'user/raceresults/' . $encrypt . "'>View Results</a>";
                    $output_arr['data'][$key][8] = '<span style=color:green;font-weight:bold;><a href="' . base_url() . 'user/racesummary/' . $encrypt . '">Live Results</a></span>';
                    $output_arr['data'][$key][10] = '<span style=color:green;font-weight:bold;><a href="' . base_url() . 'user/verifyresult/' . $encrypt . '">Verify Results</a></span>';
                }
            } else {
            ...
            if ($concate_start_date_time > $curdate) {
                $output_arr['data'][$key][6] = "<a href='" . base_url() . 'user/basketing/' . $encrypt . "'><i class='glyphicon glyphicon-plus' aria-hidden='true'></i>&nbsp;Add Entry</a><br><a href='" . base_url() . 'user/viewbasketing/' . $encrypt . "'><i class='fa fa-hand-o-right' aria-hidden='true'></i>&nbsp;View Entry</a><br><a href='" . base_url() . 'user/editbasketing/' . $encrypt . "'><i class='fa fa-pencil' aria-hidden='true'></i>&nbsp;Edit Entry</a>";
            } else if ($concate_start_date_time <= $curdate && $concate_end_date_time >= $curdate) {
                $output_arr['data'][$key][6] = "<a href='" . base_url() . 'user/viewbasketing/' . $encrypt . "'><i class='fa fa-hand-o-right' aria-hidden='true'></i>&nbsp;View Entry</a>";
            } else {
                $output_arr['data'][$key][6] = "<a href='" . base_url() . 'user/viewbasketing/' . $encrypt . "'><i class='fa fa-hand-o-right' aria-hidden='true'></i>&nbsp;View Entry</a>";
            }
            ...
            if ($output_arr['data'][$key][9] == '1') {
                $output_arr['data'][$key][9] = "<a href='" . base_url() . 'user/raceresults/' . $encrypt . "'>View Report</a>";
                $output_arr['data'][$key][count($output_arr['data'][$key]) - 1] = '';
    ...
    public function get_modal() 
    {
        ...
        if ($this->input->post('id')) 
        {
            ...
            // $get_timeZone = $this->db->query("select timezone from timezone");
            // $TimeZone = $get_timeZone->result_array();
    ...
    public function get_org_name(){
        ...
        if($postData != "")
        {
            ...
            $get_app_type = $this->db->query("select Org_code from users where Org_code = '".$apptype."'");
    ...
    public function add_edit($id = '') {
        ...
        if ($id != '') 
        {
            ...
            $data['read_type'] = $this->input->post('read_type');
            // $data['timezone'] = $this->input->post('user_country');
    ...
    public function raceeventdelete($id) {
        ...
        $ids = explode('-', $id);
        $eventides = $id;
        $get_event_name = $this->db->query("select Event_name from ppa_events where Events_id = '".$eventides."'");
        ...
        $desc = $this->session->userdata('user_details')[0]->name . " removed the list of events (" . $eventides ." ". $EventName . ")";
        $this->User_model->entrylog($action, $desc);
    ...
    public function liverace() {
        ...
        $where = "Event_date <= '" . $curdate . "' and Eventend_date >= '" . $curdate . "'";
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $curdate = date("Y-m-d h:i A");
            $curdate = strtotime($curdate);
            $get_date_time = $this->db->query("select * from ppa_events as eve LEFT JOIN ppa_event_details as eve_b ON eve.Events_id = eve_b.event_id where eve.Events_id = '" . $id . "' order by eve_b.event_id DESC limit 0,1");
            $get_date_time = $get_date_time->row_array();
            $concate_start_date_time = $get_date_time['Event_date'] . ' ' . $get_date_time['start_time'];
            $concate_start_date_time = strtotime($concate_start_date_time);
            $concate_end_date_time = $get_date_time['Eventend_date'] . ' ' . $get_date_time['end_time'];
    ...
    public function upcomingrace() {
        ...
        //$where = "auction_start_date > " . $current_date . " and auction_end_date > " . $current_date;
//where = "Event_date > '" . $current_date . "'";
    ...
    public function completedrace() {
       ...
       $curdate = date("Y-m-d");
        ...
        //$where = "Eventend_date < '" . $current_date . "'";
       $where = "Event_date < '" . $curdate . "' and Eventend_date < '" . $curdate . "'";
