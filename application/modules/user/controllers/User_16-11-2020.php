...
$filepath = base_url();
    ...
    public function changebasketstatus() {
        ...
        foreach ($selectuserlist as $key => $user) {
            ...
            if (strlen($android_deviceide) == 64) {
                $pushparameter = "";
                $badge = 1;
                $message = "Dear " . $username . " Admin approved the Bird!!";
                $title = "Bird Approved";
                $notify_key = 1;
                $GetMyDeviceId = $android_deviceide;
                // echo "<pre>GetMyDeviceId:";
                // print_r($GetMyDeviceId);"</pre>";die;
    ...
    function sendIosPushNotification($title = "", $message = "", $GetMyDeviceId = "", $pushparameter = "", $badge = 1, $apptype = "", $userreg_id = "") {
        try
        {
            //echo "<pre>title:";
            // print_r($title);"</pre>";
            ...
            // print_r($message);"</pre>";
            // echo "<pre>GetMyDeviceId:";
            // print_r($GetMyDeviceId);"</pre>";
            // echo "<pre>pushparameter:";
            // print_r($pushparameter);"</pre>";
            // echo "<pre>badge:";
            // print_r($badge);"</pre>";
            // echo "<pre>apptype:";
            // print_r($apptype);"</pre>";
            // echo "<pre>userreg_id:";
            // print_r($userreg_id);"</pre>";die;
            ...
            fclose($fp);
            //return 1;
            $logquery = $this->db->query("insert notificationlog set status ='" . $result . "',deviceid ='" . $GetMyDeviceId . "',userToken='" . $userreg_id . "',deviceType='Ios',message='" . addslashes($message) . "'");
    ...
    public function sendAnroidPushNotification($dbconnection, $message = "", $deviceToken = "", $pushparameter = "", $badge = 1, $apptype) {
        ...
        $logquery = $this->db->query("insert notificationlog set status ='" . $result . "',deviceid ='" . $deviceToken . "',userToken ='" . $userreg_id . "',deviceType='Android',message='" . addslashes($message) . "'");
        #Echo Result Of FireBase Server
        return $result;
    }
    ...
    public function updatebasket() {
        ...
        for ($ide = 0; $ide < count($ringno); $ide++) {
            ...
            //$ringnumbers.= $ringno[$ide].",";
            $selectFancier = $this->db->query("select reg_id,username,android_id,apptype, phone_no from ppa_register where reg_id='" . $fancieride . "' limit 0,1");
    ...
    public function viewresultssummary() {
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $output_arr['data'][$key][6] = number_format($calculationhour, 7); // Time Taken
    ...
    public function racesTable() {
        ...
        foreach ($output_arr['data'] as $key => $value) {
            ...
            $get_date_time = $this->db->query("select date,start_time,end_time,event_id from ppa_event_details where event_id = '" . $id . "'");
