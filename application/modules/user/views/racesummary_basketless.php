            ...
            <table border="0" cellpadding="5" cellspacing="5">
               <tr>
                ...
                <option value="Approved">Approved Photos</option>
                 ...
                 <a href="<?php echo base_url().'user/raceresults/'.$eventide;?>"><button type="button" class="btn-sm  btn btn-success" data-toggle="modal" style="font-size:14px;"><i class="fa fa-eye"></i> View Results</button></a></td>
               </tr>
            </table>
            </div> 
          </div>
...
function revertback(filenameval,raceide)
{
    var url = '<?php echo base_url();?>';//$('.content-header').attr('rel');
   $.ajax({
    type: "post",
    ...
    cache: false,   
    data: {'raceide':raceide,'img':filenameval},
    success: function(json){  
...
var url = '<?php echo base_url();?>';//$('.content-header').attr('rel');
  $(document).ready(function() {  
    ...
    { "orderable": false, "targets": 1 },
    {
          ...
          "processing": true,
          "serverSide": true,
          "ajax": {"url" : url+"user/viewresultssummary_withoutbasketing","data" : {"event":<?php echo $eventide;?>,"edit":editval,"resultvalue":"<?php echo $autoresults[0];?>","fanide":fanide} },
    ...
    setTimeout(function() {
      ...
      $('#example1_length').append('<label>&nbsp;&nbsp;&nbsp;<select onchange="getresultbyfancier(this.value)" class="form-control input-sm" id="clubtype"><option value="">All Fancier</option><?php foreach ( $geteventfancierlists as $key=>$dis ) { ?><option value="<?php echo $dis->phone_no;?>"><?php echo $dis->username;?></option><?php } ?></select></label><label>&nbsp;&nbsp;&nbsp;<select onchange="getresults(this.value)" class="form-control input-sm" id="clubtype"><?php foreach ( $racetypes as $key=>$racetypesrow ) { ?><option  value="\'<?php echo $racedetails[0]->ed_id;?>\',\'<?php echo $racetypesrow->race_distance."-".$racetypesrow->brid_type;?>\',\'<?php echo $racedetails[0]->maxi;?>\',\'<?php echo $racedetails[0]->mini;?>\',\'<?php echo $racedetails[0]->start;?>\',\'<?php echo $racetypesrow->brid_type;?>\',\'<?php echo $result_data[0]->Org_code;?>\',\'<?php echo $result_data[0]->users_id;?>\'" value="<?php echo $dis->phone_no;?>"><?php echo $racetypesrow->race_distance." - ".$racetypesrow->brid_type;?></option><?php } ?></select></label>');
...
function getresults(ed_id,dist,end,start,sttime,birdtype,orgcode,orgid)
{
   ...
   var urlval = url+"user/viewresultssummary_withoutbasketing?event="+eventides+"&resultvalue1="+sendvals+"&fanide1="+fanide;
...
function getresultbyfancier(ide)
{
   fanide = ide
   var eventides = '<?php echo $eventide;?>';
   var urlval = url+"user/viewresultssummary_withoutbasketing?event="+eventides+"&resultvalue1="+sendvals+"&fanide2="+fanide;
   $('#example1').DataTable().ajax.url( urlval ).load();
}
