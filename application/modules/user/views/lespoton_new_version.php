...
case 'forgotpassword':
    ...
    if (mysqli_num_rows($check_email) > 0) {
        ...
        $mail->Body    = "https://www.lespoton.com/portal/forgotpassword.php?verify_id=" . $str . "";
        $mail->Mailer = "smtp";
...
case 'pparegister': // Register and Login service
    ...
    $check_email = mysqli_query($dbconnection, "select * from ppa_register where email_id='" . $email_id . "' and phone_no='" . $phone_no . "' and apptype = '" . $apptype . "'");
    if (mysqli_num_rows($check_email) > 0) 
    {
        $data["status"] = 404;
    ...
    if (mysqli_num_rows($check_phone) > 0) {
        ...
        if (mysqli_num_rows($checkusercount) < $usercount) {
            ...
            if (mysqli_num_rows($selectavailability) == 0) {
                ...
                $data["userToken"] = $useride;
                if ($accdetails['usertype'] == 0 || $row["usertype"] == "0") {
                    $data["usertype"] = "Member";
                } else if($accdetails['usertype'] == 2 || $row["usertype"] == "2") {
                    $data["usertype"] = "Liberator";
                }else{
                    $data["usertype"] = "Admin";
                }
...
case 'ppalogin':
    ...
    if ($numRows == 1) {
        ...
        if (password_verify($password, $row["password"])) {
            ...
            $desc = $data["username"] . " has logged in to the app Successfull";
            $userId = $data["userId"];
            ...
            $entrylogQuery = mysqli_query($dbconnection, "select * from ppa_register where username='" . $username . "' and apptype='".$apptype."' limit 0,1");
        ...
        if ($fancierCount == 1) {
            ...
            if (move_uploaded_file($_FILES["files"]["tmp_name"], $path . $loftprefix . $path_parts['basename'])) {
                ...
                if (strlen($android_deviceide) == 64) {
                    ...
                    $badge = 1;
                    $action = "LoftUpdate";
                    sendAnroidPushNotification($dbconnection, $title, $message, $android_deviceide, $pushparameter, $badge, $apptype, $userreg_id,$fancierphone_no,$action);
                }
                ...
                if (strlen($android_deviceide) == 64) {
                    ...
                    $badge = 1;
                    $action = "LoftUpdate";
                    sendAnroidPushNotification($dbconnection, $message, $android_deviceide, $pushparameter, $badge, $apptype, $userreg_id, $notify_key, $action);
                }
...
case 'addpigeon':
    ...
    $check_race = mysqli_query($dbconnection, "select Event_date,Eventend_date,start_time,end_time from ppa_events as eve LEFT JOIN ppa_event_details as evt_d ON eve.Events_id=evt_d.event_id where Events_id='" . $race_id . "' order by evt_d.event_id DESC limit 0,1");
    ...
    $getUserType = mysqli_query($dbconnection, "select read_type from ppa_register where apptype='" . $apptype . "' and reg_id = '".$owner_id."'");
...
case 'photoApprove':
    ...
    $get_basketing_details = mysqli_query($dbconnection, "select * from ppa_basketing where ring_no = '".$ringNo."' and bird_type = '".$catgorgId."'");
...
case 'racecalculation':
    ...
    if($deviceTagId != '')
    {
        ...
        $get_bird_type = mysqli_query($dbconnection,"select bird_type,color,gender,ring_no FROM ppa_basketing where event_id='".$eventId."' and scanReader='".$deviceTagId."'");
        ...
        $get_bird_type = mysqli_query($dbconnection,"select bird_type,color,gender,ring_no FROM ppa_basketing where event_id='".$eventId."' and bird_type='".$birdId."' and fancier_id='".$userId."'");
        $bird_Type = mysqli_fetch_array($get_bird_type);
        $ring_no = "";
    }
    ...
    $miles = $getDistance * 60 * 1.1515;
    $distance = $miles * 1.609344; // Converted to Meter
    // $getKilometer = number_format($miles, 3) . " Km"; Abdur
    // $velocity = $distance / $TimeToMinutes; Abdur
    $getKilometer = number_format($distance, 7) . " Km";
    $velocity = ( $distance / $TimeToMinutes ) * 1000;
    ...
    $time_take = "";
    $time_take = number_format($TimeToMinutes, 2);
    ...
    $insert_race_details = mysqli_query($dbconnection, "insert ppa_files set device_id='" . $deviceide . "',club_code='" . $apptype . "',event_id='" . $eventId . "',temp_event_id='" . $event_details_id . "',bird_type='" . $birdId . "',username='" . $UserName . "',mobile='" . $phone_no . "',filename='" . $path_parts['basename'] . "',time_interval='" . $TravelTime[0] . "',distance='" . $distance . "',velocity='" . $velocity . "',cre_date='" . $update_date . "',owner_ringno='" . $ring_no . "',bird_gender='" . $gender . "',bird_color='" . $bird_color . "',latitude='" . $latitude . "',longitude='" . $longitude . "',tag_device_id='".$deviceTagId."'");

    if ($deviceTagId != '')
    {
        $updateReports = mysqli_query($dbconnection, "insert ppa_report set start_date='" . $event_date . "',event_sche_date='" . $event_date . "',start_time='" . $event_start_time . "',clubtype='" . $apptype . "',ring_no='" . $ring_no . "',bird_color='" . $bird_color . "',event_id='" . $eventId . "',bird_type_id='" . $birdId . "',apptype='" . $apptype . "',bird_gender='" . $gender . "',mobile_number='" . $phone_no . "',name='" . $UserName . "', device_id='" . $deviceide . "',velocity='" . $velocity . "',distance='" . $distance . "',img_name='" . $path_parts['basename'] . "',intervel='" . $TravelTime[0] . "',latitude='" . $latitude . "',longtitude='" . $longitude . "',event_details_id='" . $event_details_id . "',created_date='" . $update_date . "',device_tag='".$deviceTagId."',minis='".$time_take."'");
    }
...
case 'winnerlist':
    ...
    if($resultType == "0")
    {
        ...
        $getEventDetails = mysqli_query($dbconnection, "select * from ppa_events where Events_id='" . $eventId . "' and result_publish='".$resultType."' limit 0,1"); 
    }
        ...
        if ($checkEventDetails > 0) {
            if($resultType == "0")
            {
                //$getWinnerDetails = mysqli_query($dbconnection, "select a.*,b.latitude,b.longitude,b.profile_pic,c.date,c.start_time from ppa_files as a left join ppa_register as b on a.username = b.username left join ppa_event_details as c on a.event_id=c.event_id where a.event_id='" . $eventId . "' and a.club_code='" . $appType . "' and a.bird_type ='" . $birdtype . "' order by a.velocity DESC");
                $getWinnerDetails = mysqli_query($dbconnection, "select a.*,b.latitude,b.longitude,b.profile_pic,c.date,c.start_time from ppa_files as a left join ppa_register as b on a.username = b.username left join ppa_event_details as c on a.event_id=c.event_id where a.event_id='" . $eventId . "' and a.club_code='" . $appType . "' order by a.velocity DESC");
            }else{
            ...
            if ($WinnerDetails > 0) {
                ...
                while ($WinnerList = mysqli_fetch_array($getWinnerDetails)) {
                    ...
                    {
                        $data["winner_list"][$i]["intervel_time"] = $WinnerList["time_interval"];
                    }else{
    ...
    if ($RaceEvent > 0) 
    {
        ...
        {   
            ...
            while ($Races = mysqli_fetch_array($RaceDetails)) 
            {
                ...
                if ($Races["bird_tag_color"] == "Yellow") 
                {
                    $getdata['bird_tag_color'] = "#FFCC00";
                } 
                else if ($Races["bird_tag_color"] == "Red") 
                {
                    $getdata['bird_tag_color'] = "#FF3B30";
                } 
                else if ($Races["bird_tag_color"] == "Blue") 
                {
                    $getdata['bird_tag_color'] = "#007AFF";
                } 
                else if ($Races["bird_tag_color"] == "Green") 
                {
...
//DESC limit 0,1
    ...
    if ($userToken != "") {
        ...
        if (mysqli_num_rows($selectfanciers) > 0) {
            ...
            $getNotification = mysqli_query($dbconnection, "select * from notificationlog where userid='" . $user_id . "' order by update_date desc");
            ...
            if (mysqli_num_rows($getNotification) > 0) {
                ...
                while ($accdetails = mysqli_fetch_array($getNotification)) {
                    ...
                    $data["notification_list"][$i]["update_date"] = date("Y-m-d h:i:s A",strtotime($accdetails["update_date"]));
                    $data["notification_list"][$i]["update_date"] = $date;
...
function sendAnroidPushNotification($dbconnection, $title = "", $message = "", $android_deviceide = "", $pushparameter = "", $badge = 1, $apptype = "", $userreg_id = "",$fancierphone_no="",$action="") 
    {
        ...
        $logquery ="insert notificationlog set status ='" . $result . "',action='".$action."', deviceid ='" . $android_deviceide . "', apptype='".$apptype."', phone_no='".$fancierphone_no."', userid ='" . $userreg_id . "' , date ='" . $date . "',deviceType='Android',message='" . $msg['body'] . "'";
...
function sendIosPushNotification($dbconnection, $title = "", $message = "", $GetMyDeviceId = "", $pushparameter = "", $badge = 1, $apptype = "", $userreg_id = "",$fancierphone_no="",$action="") {
    try
    {
        $notify_key = 2;
        $passphrase = '123';
        ...
        $logquery ="insert notificationlog set status ='" . $result . "',action='".$action."', deviceid ='" . $GetMyDeviceId . "', apptype='".$apptype."', phone_no='".$fancierphone_no."', userid ='" . $userreg_id . "' , date ='" . $date . "',deviceType='IOS',message='" . $msg['body'] . "'";
        mysqli_query($dbconnection, $logquery);
