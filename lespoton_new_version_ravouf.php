    ...
    else 
    {
        ...
        if (mysqli_num_rows($checkusercount) < $usercount) 
        {
            ...
            if (mysqli_num_rows($selectavailability) == 0) 
            {
                ...
                $action = "Registered In App Successfully";
                $desc = $data["username"] . " Registered In app Successfull";
                $userId = $ide;
        ...
        {
            $action = "Fancier count limit reached.";
            $desc = $username . " Registered In App. But Fancier count limit reached for this App type '".$apptype."' and their Phone number is '".$phone_no."'";
            $userId = 0;
            entrylog($action,$desc,$userId,$dbconnection);
...
case 'bird_type_list':
    ...
    if ($userData > 0) {
        ...
        //echo "SELECT * from ppa_event_details as a LEFT JOIN ppa_events as b on a.event_id=b.Events_id LEFT JOIN ppa_bird_type as c on a.bird_id=c.b_id WHERE a.event_id='" . $event_id . "' and c.apptype='".$appType."' group by a.bird_id"; die;
...
case 'addpigeon':
    ...
    $check_race = mysqli_query($dbconnection, "select Event_date,Eventend_date from ppa_events where Events_id='" . $race_id . "'");
    if(mysqli_num_rows($check_race) > 0){
        $Race_details = mysqli_fetch_array($check_race);
        $start_date = strtotime($Race_details["Event_date"]);
        $end_date = strtotime($Race_details["Eventend_date"]);
        $curDate = date("Y-m-d");
